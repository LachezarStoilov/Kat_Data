...
# ====================================================================================
# AUTO MOTO SALES BG - CLEAN EXECUTIVE EDITION
# ====================================================================================

st.set_page_config(page_title="AUTO MOTO SALES BG", page_icon="📊", layout="wide")

# ----------------------------------------------------------------------------------
# 0. ПРЕМИУМ HERO БАНЕР И РЕСПОНСИВ ДИЗАЙН (СВЕТЛА ТЕМА)
# ----------------------------------------------------------------------------------
st.markdown("""
    <style>
...
# SVG Иконка за лого (Стилизиран Автомобил / Щит)
svg_logo = """
...
# ----------------------------------------------------------------------------------
# 1. ГЛАВНО МЕНЮ ЗА КАТЕГОРИИ (ОПТИМИЗАЦИЯ НА ПАМЕТТА)
# ----------------------------------------------------------------------------------
VEHICLE_CATEGORIES = {
...
st.markdown("### 📑 Изберете категория за анализ:")
# Използваме модерните pills бутони (от Streamlit 1.37+), за да заместим грозното select меню
...
# ----------------------------------------------------------------------------------
# 2. ОПТИМИЗИРАНА ОБРАБОТКА НА ДАННИТЕ (Memory Profiling Fix)
# ----------------------------------------------------------------------------------
SUMMARY_ROW_PATTERN = r"ОБЩ|ВСИЧК|TOTAL|SUM"

# За да не блокира Streamlit, подаваме избраната категория на функцията.
# Така тя изхвърля ненужните колони ОЩЕ ПРИ ЧЕТЕНЕТО и пести 80% от RAM паметта!
...
def load_and_process(file_bytes_list, file_names, category_name):
    ...
    for content, name in zip(file_bytes_list, file_names):
        ...
        try:
            ...
            # Намираме само колоните, които ни трябват за конкретната категория
            ...
            # Запазваме само нужните колони, за да не пълним паметта
    ...
    for item in parsed_files:
        ...
        # Оставяме само агрегираните базови колони
...
# ----------------------------------------------------------------------------------
# 3. ВРЕМЕВИ ПРОЗОРЕЦ И ПОДГОТОВКА (Sidebar)
# ----------------------------------------------------------------------------------
...
if len(p_opts) > 1:
    ...
    start_idx, end_idx = st.sidebar.select_slider(
        "Времеви прозорец:", options=p_opts, value=(default_start, default_end), 
...
def render_trend_chart(key):
    ...
    fig_trend.update_layout(title="Динамика на регистрациите във времето", template="plotly_white", height=320,
...
# ----------------------------------------------------------------------------------
# 4. ТАБОВЕ ЗА АНАЛИЗ (Изчистени имена)
# ----------------------------------------------------------------------------------
tab1, tab2, tab3 = st.tabs(["Анализ по Модели", "Пазар НОВИ МПС", "ВТОРИЧЕН Пазар"])
...
with tab1:
    ...
    sel_brand = col_f1.selectbox("1. Филтър по марка:", options=["Всички марки"] + available_brands)
    ...
    metric_tab1 = st.pills("Изследвана метрика:", options=["Нови_Рег", "Употребявани", "Пререгистрации", "Всички"], default="Всички")
...
with tab2:
    ...
    if total_new_market == 0:
        ...
        # FIX: width="stretch" вместо use_container_width[cite: 1]
